		...
		rules.Field("lines",
			...
			rules.Each(
				...
				rules.Field("taxes",
					...
					rules.Each(
						rules.Field("cat",
							rules.Assert("22", "line tax category is required", is.Present),
							rules.Assert("23", "line tax category must be a valid value",
								is.In(FuelAccountValidTaxCodes...),
							),
						),
						rules.When(is.Func("no percent", fuelAccountTaxNoPercent),
							rules.Field("rate",
								rules.Assert("24", "line tax rate is required when percent is not set", is.Present),
							),
						),
						rules.Field("rate",
							rules.AssertIfPresent("25", "line tax rate must be greater than 0", num.Positive),
						),
						rules.Field("amount",
							rules.Assert("26", "line tax amount must be greater than 0", num.Positive),
						),
					),
				),
			),
