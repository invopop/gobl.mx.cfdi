package addon_test

import (
	"testing"

	"github.com/invopop/gobl.cfdi/addon"
	"github.com/invopop/gobl/norm"
	"github.com/invopop/gobl/pay"
	"github.com/invopop/gobl/rules"
	"github.com/invopop/gobl/tax"
	"github.com/stretchr/testify/assert"
)

func TestPaymentMeansExtensions(t *testing.T) {
	ext := addon.PaymentMeansExtensions()
	assert.Equal(t, "01", ext.Get(pay.MeansKeyCash).String())
}

func TestNormalizePayInstructions(t *testing.T) {

	t.Run("nil", func(t *testing.T) {
		var instr *pay.Instructions
		assert.NotPanics(t, func() {
			norm.Normalize(instr, tax.AddonContext(addon.V4))
		})
	})

	t.Run("with match", func(t *testing.T) {
		instr := &pay.Instructions{
			Key: pay.MeansKeyOnline.With(addon.MeansKeyWallet),
		}
		norm.Normalize(instr, tax.AddonContext(addon.V4))
		assert.Equal(t, "05", instr.Ext.Get(addon.ExtKeyPaymentMeans).String())
	})
}

func TestNormalizePayAdvance(t *testing.T) {

	t.Run("nil", func(t *testing.T) {
		var adv *pay.Record
		assert.NotPanics(t, func() {
			norm.Normalize(adv, tax.AddonContext(addon.V4))
		})
	})

	t.Run("with match", func(t *testing.T) {
		adv := &pay.Record{
			Key: pay.MeansKeyOnline.With(addon.MeansKeyWallet),
		}
		norm.Normalize(adv, tax.AddonContext(addon.V4))
		assert.Equal(t, "05", adv.Ext.Get(addon.ExtKeyPaymentMeans).String())
	})
}

func TestValidatePayTerms(t *testing.T) {
	t.Run("nil", func(t *testing.T) {
		var terms *pay.Terms
		assert.NotPanics(t, func() {
			assert.NoError(t, rules.Validate(terms, tax.AddonContext(addon.V4)))
		})
	})

	t.Run("valid", func(t *testing.T) {
		terms := &pay.Terms{
			Notes: "test",
		}
		err := rules.Validate(terms, tax.AddonContext(addon.V4))
		assert.NoError(t, err)
	})
}
