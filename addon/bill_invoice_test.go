...
func validInvoiceGlobal() *bill.Invoice {
	inv := validInvoice()
	inv.Tags = tax.WithTags(addon.TagGlobal)
	inv.Lines[0].Item.Ref = "TEST1234"
	inv.Tax.Ext = inv.Tax.Ext.Merge(tax.ExtensionsOf(cbc.CodeMap{
		addon.ExtKeyGlobalPeriod: "04",
		addon.ExtKeyGlobalMonth:  "01",
		addon.ExtKeyGlobalYear:   "2025",
	}))
...
func TestNormalizeInvoice(t *testing.T) {
	t.Run("no tax", func(t *testing.T) {
		inv := validInvoice()
		inv.Addons = tax.WithAddons(addon.V4)
		require.NoError(t, inv.Calculate())
		require.NoError(t, rules.Validate(inv))
		require.NotNil(t, inv.Tax)
		assert.Equal(t, cbc.Code("21000"), inv.Tax.Ext.Get(addon.ExtKeyIssuePlace))
	})
...
func TestInvoiceLineItemValidation(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			tags: tax.WithTags(addon.TagGlobal),
			item: &org.Item{
				Ref:   "TEST1234",
				Price: num.NewAmount(10000, 2),
				Name:  "Test purchase",
